...
class Board extends StatefulWidget {
  ...
  Board.fromPredefinedCells({key, required List<List<Cell>> cells})
...
class BoardState extends State<Board> {
  ...
  resetCells() {
    this._cells = widget._initialCells ??
        List.generate(this.widget._columns,
            (col) => List.generate(this.widget._rows, (row) => Cell.fromRandomLifeStatus()));
  }

  updateCells() {
    // Clonamos las celdas (por valor, en vez de por referencia)
    List<List<Cell>> updatedCells =
        List<List<Cell>>.of(this._cells.map((e) => e.map<Cell>((e) => Cell(e.isAlive)).toList()));
  ...
  @override
  bool operator ==(other) =>
      other is Board && this.widget._columns == other._columns && this.widget._rows == other._rows;

  int get hashCode =>
      this.cells.hashCode * this.widget._columns.hashCode * this.widget._rows.hashCode;
    ...
    return Column(
        children: List.generate(
            ...
            (col) => Row(
                  ...
                  children: List.generate(
                      widget._rows, (row) => this._buildCell(context, this._cells[col][row])),
                )));
  }
